...
const useDietData = (userId) => {
  ...
  const recalculateGoals = useCallback(async () => {
    ...
    try { const today = new Date().toISOString().slice(0, 10); const { error } = await supabase.rpc('fn_calc_goals', { p_user_id: userId, p_date: today })
      if (error) throw error; toast.success('Metas recalculadas com sucesso! 🔥'); await fetchAllData()
  ...
  const addFood = useCallback(async (foodData) => {
    ...
    try { const today = new Date().toISOString().split('T')[0]; const qty = parseFloat(quantity) || 0; const gramsPerUnit = selectedFood.grams_per_unit || 1; const gramsTotal = qty * gramsPerUnit
      const { error } = await supabase.from('meal_items').insert({ user_id: userId, date: today, meal_type: selectedMealType, food_id: selectedFood.id, qty_units: qty, grams_total: gramsTotal, })
      if (error) throw error; toast.success('Alimento adicionado! 🎉'); await fetchAllData()
  ...
  const updateFood = useCallback(async (itemId, foodData) => {
    const { selectedMealType, selectedFood, quantity } = foodData
    try { const qty = parseFloat(quantity) || 0; const gramsPerUnit = selectedFood.grams_per_unit || 1; const gramsTotal = qty * gramsPerUnit
      const { error } = await supabase.from('meal_items').update({ meal_type: selectedMealType, food_id: selectedFood.id, qty_units: qty, grams_total: gramsTotal, }).eq('id', itemId)
      if (error) throw error; toast.success('Alimento atualizado! ✅'); await fetchAllData()
    } catch (error) { console.error('Error updating food:', error); toast.error('Erro ao atualizar alimento.') }
  }, [userId, fetchAllData])

  const deleteFood = useCallback(async (itemId) => {
    try { const { error } = await supabase.from('meal_items').delete().eq('id', itemId)
      if (error) throw error; toast.success('Alimento removido! 🗑️'); await fetchAllData()
    } catch (error) { console.error('Error deleting food:', error); toast.error('Erro ao remover alimento.') }
  }, [userId, fetchAllData])
      ...
      <Card className={cardStyles.interactive} onClick={onToggle}>
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h3 className="font-bold text-lg flex items-center gap-2 text-foreground"><span>{meal.emoji}</span> {meal.name}</h3>
            ...
            <div className="flex justify-between text-sm items-center"><span className="font-medium text-foreground">{item.food_name}</span>
              <div className="flex items-center gap-2"><span className="font-semibold text-foreground">{item.item_kcal} kcal</span>
                <div className="flex gap-1 opacity-0 group-hover/item:opacity-100 transition-opacity">
                  <Button size="icon" variant="ghost" className="h-7 w-7 text-muted-foreground hover:text-blue-600" onClick={(e) => { e.stopPropagation(); onEditItem(item); }}><Edit2 className="w-4 h-4" /></Button>
                  <Button size="icon" variant="ghost" className="h-7 w-7 text-muted-foreground hover:text-red-600" onClick={(e) => { e.stopPropagation(); onDeleteItem(item.id); }}><Trash2 className="w-4 h-4" /></Button>
                </div>
              </div>
            </div></div>
...
export default function DietPlanner() {
  ...
  const calorieProgress = useMemo(() => { if (!dailyIntake) return 0; return ((dailyIntake.total_kcal || 0) / (dailyIntake.goal_kcal || 2000)) * 100 }, [dailyIntake])
  ...
  const getMotivationalMessage = (progress) => { if (progress >= 100) return '🔥 Meta atingida! Você é lendário.'; if (progress >= 90) return `🌟 Quase lá! Sua consistência é incrível.`; if (progress >= 70) return `💪 Excelente! Você está no caminho certo.`; if (progress >= 50) return `👍 Bom progresso! Continue assim.`; return `🌅 Vamos começar! Cada passo conta.`; }
  ...
  const handleAddFood = async (foodData) => { await actions.addFood(foodData) }
  const handleUpdateFood = async (itemId, foodData) => { await actions.updateFood(itemId, foodData) }
  const handleEditClick = (item) => { setItemToEdit(item); setIsFoodModalOpen(true) }
  const handleDeleteClick = (itemId) => { actions.deleteFood(itemId) }

  if (loading) { return (<div className="flex items-center justify-center min-h-screen"><motion.div animate={{ rotate: 360 }} transition={{ duration: 1, repeat: Infinity, ease: "linear" }}><Flame className="w-12 h-12 text-phoenix-500" /></motion.div></div>) }
